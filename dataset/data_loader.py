...
#                     import pdb; pdb.set_trace()
                    ...
                    track_path=os.path.join(args.face_path,uid, trackid)
...
class ImagerLoader(torch.utils.data.Dataset):
    def __init__(self, source_path, file_name, json_path, gt_path,
        ...
        self.face_path = args.face_path
        self.transform = transform

        if mode=="train":
            if args.mesh:
                with open(os.path.join(args.prior_path,"mesh_pose_train.json"),'r') as f:
        ...
        if args.Gaussiandrop and mode=='train':
            file=open(os.path.join(args.prior_path,'train_var.json'),'r')
#             file=open(os.path.join(args.prior_path,'train_filter_all.json'),'r')
        ...
        if mode=='train':
            ...
            self.dic3=None
            file=open(os.path.join(args.prior_path,'val_headpose.json'),'r')
...
class TestImagerLoader(torch.utils.data.Dataset):
    def __init__(self, test_path,args,stride=1, transform=None):
        ...
        if self.args.mesh:
            with open(os.path.join(args.prior_path,"mesh_pose_test.json"),"r") as f:
                self.head2D_dict=json.load(f)
        else:
            self.head2D_dict=None
