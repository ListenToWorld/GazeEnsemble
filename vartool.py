...
def process(imgs,kframes,face_path):
    ...
    with open(f"{mode}_var.json","w") as f:
...
if(mode=="train"):
    ...
    file = args.val_file
train_dataset = ImagerLoader(args.source_path, file, args.json_path, args.gt_path,
                                   stride=1, mode=mode, transform=get_transform(False),args=args)
process(train_dataset.imgs,train_dataset.kframes,train_dataset.face_path)


with open(f"{mode}_var.json","r") as f:
...
with open(f"{mode}_p.json","w") as f:
    json.dump(p_dict,f)